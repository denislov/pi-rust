...
- [~] Add a confirmation boundary for write-capable, team, or high-cost delegation when policy requires it. Current authorization decisions mark `Always` and team delegation under `Writes` as `RequiresConfirmation`, store those requests in an in-memory pending queue, and expose RPC `list_delegation_confirmations`/`approve_delegation`/`reject_delegation` plus interactive `/delegations` and `/delegation approve|reject`; richer confirmation prompts plus durable persistence/expiry policy remain follow-up.
