...
- [~] Add RPC command support for listing profiles, switching default profile, invoking one-off agent work, and invoking team work. `list_agent_profiles` and `list_team_profiles` now expose session-owned registry state, and `set_default_agent_profile` updates the session-owned default profile; one-off agent/team invocation commands remain follow-up.
