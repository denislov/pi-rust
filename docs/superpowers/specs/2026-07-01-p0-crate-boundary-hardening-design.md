...
- Shared product operations such as manual compaction, branch summary, export, session switch, plugin command execution, UI/keybind dispatch, delegation-first helper orchestration, explicit team workflows, and self-healing edits should be implemented as session-owned services or product Flows.
