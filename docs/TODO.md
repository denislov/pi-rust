...
- [~] Add Phase 4 tests. `agent_turn_flow` integration coverage verifies `AgentTurnContext` construction from existing `Agent` state without queue drainage, `PrepareContextNode` provider-request construction against the current `Agent` snapshot behavior, runtime compaction node context/event updates, provider stream success/error event buffering, decide-stop-or-tools text/tool actions, tool execution success/unknown-tool behavior, parallel tool end-event/message ordering, before/after tool hooks, update callback events, and terminate completion.
