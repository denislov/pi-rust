...
- [~] Add plugin capability reporting. Internal `PluginCapabilities` reports tool/command/hook/UI/keybind/FlowExtension provider counts and diagnostic counts from `PluginService`, and `CapabilityService` now reports the plugin kernel as available instead of unsupported.
- [~] Add `ToolProvider`. Internal Rust trait exists and returns `AgentTool` values through `PluginService::collect_tools()`.
- [~] Add `CommandProvider`. Internal Rust trait, `CommandDefinition`, registration host, registry storage, and `PluginService::collect_commands()` exist; slash/RPC execution and adapter capability exposure remain later slices.
- [~] Add `HookProvider`. Internal Rust trait, `HookRegistration`, `PromptHookPoint`, `HookFailurePolicy`, scoped `PromptHookContext`, `HookOutcome`, registration host, registry storage, collection, and isolated execution through `PluginService::run_prompt_hook()` exist; public/plugin author API hardening remains later.
...
- [~] Integrate prompt hooks through `PromptTurnFlow`. Initial hook calls are wired at prompt prepare, input prepared, resources loaded, before/after agent turn, before session commit readiness, and after prompt completion. `AfterSessionCommit` currently runs before owner-level durable session finalization results are observable.
- [~] Add plugin failure isolation. Tool/command/hook/UI/keybind/FlowExtension provider `PluginError` values, collection-boundary panics, and hook execution panics/errors are captured as plugin diagnostics or fail-closed `CodingSessionError::Plugin` values while healthy providers still contribute capabilities.
- [~] Add Phase 5 tests. Unit coverage verifies tool collection, command collection, hook collection, UI action collection, keybinding collection, FlowExtension point collection/failure isolation, prompt hook execution diagnostics, fail-open hook errors, fail-closed hook errors, plugin capability reporting, returned-error isolation, panic isolation, and RuntimeService provider-context merging.
- [~] Run Phase 5 focused checks. Latest verified subset: `cargo fmt --check`, `cargo test -p pi-coding-agent collect_flow_extension_points`, `cargo test -p pi-coding-agent plugin`, `cargo test -p pi-coding-agent coding_session`, `cargo check --workspace`, and `cargo test -p pi-coding-agent` pass for the FlowExtension boundary slice. Previous subsets covered UI/keybind and prompt-hook execution checks.
