...
- [~] Add plugin registry module. Initial internal `crate::plugins::PluginRegistry` stores Rust `ToolProvider`, `CommandProvider`, `HookProvider`, `UiProvider`, and `KeybindProvider` instances with plugin metadata/source types; FlowExtension registry remains open.
...
- [~] Add plugin capability reporting. Internal `PluginCapabilities` reports tool/command/hook/UI/keybind provider counts and diagnostic counts from `PluginService`, and `CapabilityService` now reports the plugin kernel as available instead of unsupported.
...
- [~] Add plugin failure isolation. Tool/command/hook/UI/keybind provider `PluginError` values, collection-boundary panics, and hook execution panics/errors are captured as plugin diagnostics or fail-closed `CodingSessionError::Plugin` values while healthy providers still contribute capabilities.
- [~] Add Phase 5 tests. Unit coverage verifies tool collection, command collection, hook collection, UI action collection, keybinding collection, prompt hook execution diagnostics, fail-open hook errors, fail-closed hook errors, plugin capability reporting, returned-error isolation, panic isolation, and RuntimeService provider-context merging.
- [~] Run Phase 5 focused checks. Latest verified subset: `cargo fmt --check`, `cargo test -p pi-coding-agent collect_ui_actions`, `cargo test -p pi-coding-agent collect_keybindings`, `cargo test -p pi-coding-agent capabilities_report_registered_plugin_capabilities`, `cargo test -p pi-coding-agent plugin`, `cargo test -p pi-coding-agent coding_session`, and `cargo check --workspace` pass for the UI/keybind boundary slice. Previous broader subset: `cargo test -p pi-coding-agent prompt_turn_flow`, `cargo test -p pi-coding-agent`, and `git diff --check` passed for the prompt-hook execution slice.
