...
- [~] Implement Phase 4: introduce `AgentTurnFlow` in `pi-agent-core`. Initial `agent_turn_flow` module, `AgentTurnContext` snapshot boundary, `PrepareContextNode`, runtime compaction node, provider stream node, decide-stop-or-tools node, and initial tool execution node are in place without changing `Agent::run()` behavior.
...
- [~] Keep old `session_runner` as transitional wrapper. It remains the execution source for legacy JSONL interactive session-target fallbacks and other explicitly unmigrated legacy paths while print, JSON, RPC, primary interactive prompt execution, and Rust-native manual compaction route through `CodingAgentSession`/`CodingAgentEvent`.
...
- [~] Extract tool execution node. `ExecuteToolsNode` now handles pending tool calls in assistant order, emits `ToolCallStart`/`ToolCallEnd`, executes matching tools, records unknown tools as error results, appends `ToolResult` messages, and returns `continue`; parallel execution, before/after hooks, update callbacks, and terminate behavior still need parity coverage.
- [ ] Preserve `AgentEvent` behavior.
- [ ] Make `Agent::run()` delegate to `AgentTurnFlow`.
- [~] Add Phase 4 tests. `agent_turn_flow` integration coverage verifies `AgentTurnContext` construction from existing `Agent` state without queue drainage, `PrepareContextNode` provider-request construction against the current `Agent` snapshot behavior, runtime compaction node context/event updates, provider stream success/error event buffering, decide-stop-or-tools text/tool actions, and initial tool execution success/unknown-tool behavior.
- [~] Run Phase 4 focused checks. Latest verified subset: `cargo fmt --check`, `cargo test -p pi-agent-core --test agent_turn_flow`, `cargo test -p pi-agent-core --test agent_loop`, `cargo test -p pi-agent-core --test compaction`, `cargo test -p pi-agent-core --test parallel_tools`, `cargo test -p pi-agent-core --test hooks`, and `cargo check --workspace`.
