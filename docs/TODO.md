...
- [~] Add plugin registry module. Initial internal `crate::plugins::PluginRegistry` stores Rust `ToolProvider`, `CommandProvider`, and `HookProvider` instances with plugin metadata/source types; UI/keybind/flow registries remain open.
- [~] Add capability-scoped plugin hosts. Initial `ToolRegistrationHost`, `CommandRegistrationHost`, and `HookRegistrationHost` establish the scoped-host pattern without exposing `CodingAgentSession`, `SessionService`, auth, filesystem, shell, or Flow internals.
- [~] Add plugin capability reporting. Internal `PluginCapabilities` reports provider/diagnostic counts from `PluginService`, and `CapabilityService` now reports the plugin kernel as available instead of unsupported.
...
- [~] Add plugin failure isolation. Tool/command/hook provider `PluginError` values, collection-boundary panics, and hook execution panics/errors are captured as plugin diagnostics or fail-closed `CodingSessionError::Plugin` values while healthy providers still contribute capabilities.
- [~] Add Phase 5 tests. Unit coverage verifies tool collection, command collection, hook collection, prompt hook execution diagnostics, fail-open hook errors, fail-closed hook errors, plugin capability reporting, returned-error isolation, panic isolation, and RuntimeService provider-context merging.
- [~] Run Phase 5 focused checks. Latest verified subset: `cargo fmt --check`, `cargo test -p pi-coding-agent prompt_turn_flow_runs_noncritical_prompt_hooks_as_diagnostics`, `cargo test -p pi-coding-agent prompt_turn_flow_continues_for_fail_open_hook_error_as_diagnostic`, `cargo test -p pi-coding-agent prompt_turn_flow_aborts_for_fail_closed_hook_error`, `cargo test -p pi-coding-agent plugin`, `cargo test -p pi-coding-agent prompt_turn_flow`, `cargo test -p pi-coding-agent coding_session`, `cargo check --workspace`, `cargo test -p pi-coding-agent`, and `git diff --check` pass for the prompt-hook execution slice. Previous broader subset: `cargo test -p pi-coding-agent collect_prompt_hooks` and `cargo test -p pi-coding-agent capabilities_report` passed for earlier Phase 5 slices.
