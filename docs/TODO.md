...
- [~] Add capability-scoped plugin hosts. Initial `ToolRegistrationHost`, `CommandRegistrationHost`, `HookRegistrationHost`, `UiRegistrationHost`, and `KeybindRegistrationHost` establish the scoped-host pattern without exposing `CodingAgentSession`, `SessionService`, auth, filesystem, shell, or Flow internals.
...
- [x] Reserve first-party `FlowExtension`. Internal `FlowExtension` exposes only a closed `FlowExtensionPoint` enum and provider-owned point collection through `PluginService`; it does not expose Flow graph rewrites, node insertion, Lua registration, session storage, auth, shell, or filesystem access.
...
- 2026-07-01: Phase 5 FlowExtension boundary reserved. `FlowExtension` now exposes only named `FlowExtensionPoint` values, registry storage, capability counts, and collection/failure-isolation through `PluginService`; arbitrary Flow graph rewrites and Lua mutation hooks remain unavailable.
