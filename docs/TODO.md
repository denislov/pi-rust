...
- [~] Implement Phase 3: converge CLI/RPC/interactive adapters. Concrete `CodingAgentCapabilities`/`CapabilityStatus` model, RPC `get_state` capability reporting, RPC `CodingAgentEvent` adapter, enabled plus disabled-session RPC prompt routing, primary interactive prompt routing, JSON execution routing, Rust-native resume hydration, and adapter-provided cwd recording/filtering are in place; migrated session actions remain.
