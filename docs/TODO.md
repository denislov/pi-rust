...
- [~] Add `HookProvider`. Internal Rust trait, `HookRegistration`, `PromptHookPoint`, `HookFailurePolicy`, registration host, registry storage, and `PluginService::collect_prompt_hooks()` exist; PromptTurnFlow hook execution remains the next slice.
...
- [~] Add plugin failure isolation. Tool/command/hook provider `PluginError` values and collection-boundary panics are captured as plugin diagnostics while healthy providers still contribute capabilities.
- [~] Add Phase 5 tests. Unit coverage verifies tool collection, command collection, hook collection, plugin capability reporting, returned-error isolation, panic isolation, and RuntimeService provider-context merging.
- [~] Run Phase 5 focused checks. Latest verified subset: `cargo fmt --check`, `cargo test -p pi-coding-agent collect_prompt_hooks`, `cargo test -p pi-coding-agent capabilities_report`, `cargo test -p pi-coding-agent plugin`, and `cargo check --workspace` pass for the hook-provider boundary slice. Previous broader subset: `cargo test -p pi-coding-agent coding_session` and `cargo test -p pi-coding-agent` passed for the registry/tool-provider/command-provider/runtime slice.
...
- 2026-07-01: Phase 5 hook provider boundary started. `HookProvider`, `HookRegistration`, `PromptHookPoint`, `HookFailurePolicy`, `HookRegistrationHost`, registry storage, hook collection, capability counts, and provider failure diagnostics are in place; PromptTurnFlow hook execution remains next.
