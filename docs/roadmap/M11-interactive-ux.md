...
- slash command registry 已覆盖 TS built-in 命令名（23 个），`/help`、`/quit`、`/name`、`/session`、`/hotkeys`、`/model` 具备本地行为；依赖外设/会话后端的命令返回显式未实现提示，留给对应里程碑接管后端行为。
...
- ⏭️ 其余会话/外设命令（`/export`、`/import`、`/share`、`/copy`、`/login`、`/logout` 等）的后端实现归 M13。
...
- 交互模式 Ctrl+P/Ctrl+Shift+P 模型循环切换待实现。
