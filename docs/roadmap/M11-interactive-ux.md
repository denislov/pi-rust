...
- slash command registry 已覆盖 TS built-in 命令名（23 个），`/help`、`/quit`、`/name`、`/session`、`/hotkeys`、`/model`、`/copy`、`/export`、`/new`、`/reload` 具备本地行为；其余依赖外设/会话后端的命令返回显式未实现提示，留给对应里程碑接管后端行为。
...
- ✅ `/help`、`/quit`、`/name`、`/session`、`/hotkeys`、`/model` 已有本地行为。
- ⏭️ 其余会话/外设命令（`/import`、`/share`、`/login`、`/logout`、`/fork`、`/clone`、`/tree`、完整 HTML 导出 parity 等）的后端实现归 M13/后续交互 polish。
