...
| M4 | 进行中 | WP4.3 已移除 capability/event/flow/client-projection/session-log 及各 operation Flow 的文件级 dead-code suppression；无 production consumer 的 capability/Flow/client 表面已删除，仅测试使用的构造、观察、取消验证、durable fixture 与 execution-env adapters 精确限制为 `cfg(test)`。旧 `ClientConnection`/`SubmittedOperation` 投影模型及 legacy connector 已由 SnapshotCoordinator/public connection 合同取代并删除（`3208364`）；session-log durable schema/store/replay/transaction 生产路径保留（`ec447f7`）。WP4.2 删除未实现的 `CancelMatchingOperations` revocation mode（`0730e44`）。 | 完成最后的 `prompt.rs` file-level suppression，审计 raw service capability escape，完成 generation/revocation 与 snapshot audit 统一 |
...
| M6-M7 | M6 进行中；M7 未开始 | M6/WP6.2 的 session facade/module split 已覆盖 query、dispatcher、connection、lifecycle、submission、control、admission/capability、prompt 与其余 operation helpers；construction options、profile/plugin/runtime resolution、cwd 与 replay-derived owner state 也已归入 `session_lifecycle.rs`（`92a0920`）。`coding_session/mod.rs` 仅保留 module graph、facade exports/import aliases 与 session state layout；legacy internal client facade、submission projection 与无消费者 ClientService pass-through 已删除（`3208364`）。WP6.3 已将 session tests 与 fixture bridges 分离到独立 test-only owners。 | 完成最后的 prompt suppression、hidden fallback 与 facade 审计；随后生成 API diff/migration guide 并进入 release train |
