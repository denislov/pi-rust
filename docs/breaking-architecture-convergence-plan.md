...
| M6-M7 | M6 进行中；M7 未开始 | M6/WP6.2 的 session facade/module split 已覆盖 query、dispatcher、connection、lifecycle、submission、control、admission/capability、prompt 与其余 operation helpers；construction options、profile/plugin/runtime resolution、cwd 与 replay-derived owner state 也已归入 `session_lifecycle.rs`（`92a0920`）。legacy internal client facade、submission projection 与无消费者 pass-through 已删除（`3208364`）；file-level suppression 已为零（`141b184`）。WP6.3 已将 session tests 与 fixture bridges 分离；`0.1 → 0.2` API diff 和 migration guide 已生成（`8d92e4c`）。production `cfg(any())` fallback 已由递归 boundary guard 禁止，RPC state 中永久禁用且引用旧 submission 模型的测试模块已删除。 | 删除剩余 root/doc-hidden compatibility modules，完成 machine-readable API snapshot，再进入 release train |
...
当前 hidden-fallback 结论：production `pi-coding-agent/src` 不含永久禁用分支，且 boundary guard 会在后续引入 `cfg(any())` 时失败；`#[allow(deprecated)]` 仅存在于 debug/test-harness owner，不构成 production compatibility dispatcher。尚未满足的主要退出项是 root `#[doc(hidden)] pub mod` compatibility surface 与 machine-readable public API snapshot。
