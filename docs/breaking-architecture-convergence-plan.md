...
| M6-M7 | M6 进行中；M7 未开始 | M6/WP6.2 的 session facade/module split 已覆盖 query、dispatcher、connection、lifecycle、submission、control、admission/capability、prompt 与其余 operation helpers；construction options、profile/plugin/runtime resolution、cwd 与 replay-derived owner state 也已归入 `session_lifecycle.rs`（`92a0920`）。legacy internal client facade、submission projection 与无消费者 pass-through 已删除（`3208364`）；file-level suppression 已为零（`141b184`）。WP6.3 已将 session tests 与 fixture bridges 分离；`0.1 → 0.2` API diff 和 migration guide 已生成（`8d92e4c`）。production `cfg(any())` fallback 已由递归 boundary guard 禁止。9 个 root utility modules 已收窄为 private，剩余 doc-hidden modules 从 15 个降至 6 个（`config/input/interactive/protocol/theme/tools`）。 | 收敛剩余 6 个 root/doc-hidden modules 的测试消费者，完成 machine-readable API snapshot，再进入 release train |
...
当前 hidden-fallback 结论：production `pi-coding-agent/src` 不含永久禁用分支，且 boundary guard 会在后续引入 `cfg(any())` 时失败；`#[allow(deprecated)]` 仅存在于 debug/test-harness owner，不构成 production compatibility dispatcher。root compatibility modules 已从 15 个降至 6 个；尚未满足的主要退出项是这 6 个模块的内部 integration-test 归属收敛与 machine-readable public API snapshot。
