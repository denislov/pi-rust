...
| M6-M7 | M6 进行中；M7 未开始 | M6/WP6.2 的 session facade/module split 已完成；production hidden fallback 已由 guard 禁止。`pi-coding-agent` 的 15 个 root implementation modules 已全部 private，interactive/tools 行为测试已迁回 owner-crate internal targets（`01838d3`）。`pi-agent-core` 的最后 7 个 doc-hidden modules 也已 private，`api` 成为唯一 public root module（`ef562f7`）。 | 收敛 `pi-ai` 剩余 doc-hidden modules，完成 machine-readable API snapshot，再进入 release train |
...
- RPC hidden fallback audit：确认 production source 不存在 `cfg(any())` 分支，删除 `rpc/state.rs` 中永久禁用且仍引用已退役 client submission 状态的测试模块，并新增递归 production-source guard 防止永久禁用 fallback 回归；`prompt.rs` 内其余永久禁用 replay tests 属 test-only item-level 历史片段，按当前清理范围可跳过且不计入 production exit gate。
- `4b6b52e`：将 `args/error/models/print_mode/prompt_options/request/resources/session` root modules 收窄为 private，迁移合法 integration consumers 到 stable `api`，增加 external compile-fail 与 source guards；visibility 收窄后删除两个无消费者 utility functions。
- `1b12d86`：迁移 CLI/JSON/session CLI tests 到 stable runtime facade，并将 root `runtime` module 收窄为 private。
- `8ee2a69`：将 configuration/settings/auth 的 supported slice 明确加入 stable facade，迁移 config/runtime tests，并将 root `config` module 收窄为 private。
- `23edde4`：将 prompt input processing contracts 加入 stable facade，迁移 resource/input tests，并将 root `input` module 收窄为 private。
- `eea9f90`：补齐 theme parsing、terminal detection、reload、syntax highlighting、export 与内置 schema/assets facade，迁移 theme tests，并将 root `theme` module 收窄为 private。
- `a3b5f78`：补齐 protocol event adapter、JSONL、RPC runner、lifecycle wire type 与 version facade，迁移 binary/protocol tests，并将 root `protocol` module 收窄为 private。
- `01838d3`：将约 6k 行 interactive/tools 实现行为测试从外部 integration targets 迁入 owner-crate `src/internal_tests`，共享同一个 env/provider guard，并把最后两个 root modules 收窄为 private；删除 visibility 收窄后暴露的死 wrapper/variant，将仅测试调用路径精确限制为 `cfg(test)`。
- `ef562f7`：确认 `pi-agent-core` 最后 7 个 doc-hidden modules 无跨 crate production consumer，将 agent/env/errors/harness/hooks/queues 收窄为 private；补齐四个 message conversion/context assembly facade functions，迁移测试并将 boundary guard 提升为只允许 `api` 一个 public root module。
- `8d92e4c`：新增 `docs/0.1-to-0.2-api-diff.md` 与 `docs/0.2-migration-guide.md`，覆盖三 crate facade/import breaks、typed ProductEvent、versioned snapshot cursor、RPC `afterSnapshotCursor`、submitted status/revocation/error removals及 plugin Flow Non-Goal；明确 M7 前 manifests/protocol constants 仍为 0.1/major 1。
...
当前 hidden-fallback 结论：production `pi-coding-agent/src` 不含永久禁用分支，且 boundary guard 会在后续引入 `cfg(any())` 时失败；`#[allow(deprecated)]` 仅存在于 test-support owner，不构成 production compatibility dispatcher。`pi-coding-agent` 与 `pi-agent-core` 均只保留 `api` public root module；external compile-fail、source visibility 和 stable facade guards 同时防止回归。machine-readable public API snapshot 以及 `pi-ai` 剩余 doc-hidden modules 的最终分类仍待完成。
