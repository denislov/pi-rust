...
| M6-M7 | M6 进行中；M7 未开始 | M6/WP6.2 已将 query、dispatcher、connection/snapshot/shutdown、construction/static lifecycle、submission、prompt control 与 operation admission/capability helpers 分别移入专门 owner modules；`coding_session/mod.rs` production owner 已降至 867 行。WP6.3 已清除无生产消费者的 replay seam，并将原 5115 行内联 session tests 移入显式 `cfg(test)` 的 `session_tests.rs`（`bf300d7`），production source inventory 不再混入测试调用。 | 继续迁移 operation-specific helpers，完成 facade/dead-code/fallback 审计与剩余 test-support 收敛；随后进入 release train |
