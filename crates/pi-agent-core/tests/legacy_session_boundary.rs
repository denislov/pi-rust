...
fn session_context_sources_live_outside_legacy_session_module() {
    ...
    let session_context_dir = crate_dir.join("src/session_context");
    for relative in ["context.rs", "error.rs", "memory.rs"] {
        assert!(
            session_context_dir.join(relative).is_file(),
            "session-context source should live under src/session_context/{relative}"
        );
    }
}
