...
use pi_agent_core::api::testing::{
    ...
    BeforeProviderPayloadPatch, BeforeProviderRequestPatch, HeaderPatch, InMemoryExecutionEnv,
    Patch, ProviderAuth, ProviderResponse, StreamOptionsPatch,
};
use pi_ai::api::conversation::{AssistantMessage, ContentBlock, Context, Message, StopReason};
use pi_ai::api::hooks::ProviderResponseInfo;
...
async fn provider_request_auth_and_patch_merge_delete_apply_to_each_provider_call() {
    ...
    let harness = AgentHarness::new(config).with_hooks(hooks);
    harness.add_tool(pi_agent_core::api::tool::AgentTool::new_text(
        "noop",
        "no operation",
        ...
        fn stream(&self, model: &Model, _ctx: Context, opts: Option<StreamOptions>) -> EventStream {
            ...
            Box::pin(async_stream::stream! {
                ...
                if let Some(hooks) = hooks.as_ref() {
                    ...
                    hooks.emit_response(ProviderResponseInfo {
                        status: Some(202),
