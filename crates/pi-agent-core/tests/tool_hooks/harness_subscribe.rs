...
use pi_agent_core::api::testing::{AgentHarness, AgentHarnessEvent, AgentHarnessHooks};
...
async fn on_context_appends_to_existing_hook_chain() {
    use pi_agent_core::api::testing::on_kind::ContextKind;
...
async fn phase_starts_idle_and_returns_to_idle_after_prompt() {
    use pi_agent_core::api::testing::AgentHarnessPhase;
...
async fn abort_returns_cleared_queue_messages() {
    use pi_agent_core::api::testing::AbortResult;
    ...
    let texts: Vec<&str> = result
        ...
        .filter_map(|m| match m {
            pi_agent_core::api::agent::AgentMessage::UserText { text, .. } => Some(text.as_str()),
            _ => None,
        })
        .collect();
