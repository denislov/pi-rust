...
use pi_agent_core::api::agent::{
    AfterToolCallResult, Agent, AgentEvent, AgentLoopTurnUpdate, AgentMessage, AgentResources,
    BeforeProviderRequestResult, BeforeToolCallResult, CompactionConfig, CompactionSettings,
    QueueMode,
};
use pi_agent_core::api::flow::{Action, Flow};
...
#[tokio::test]
async fn prepare_provider_request_node_builds_provider_request_from_context_snapshot() {
    let mut config = common::agent_config(common::faux_model("test-api"));
    config.system_prompt = Some("system rules".into());
    ...
    assert_eq!(outcome.last_node.as_str(), "prepare_provider_request");
    let request = context
        .provider_request
        .as_ref()
...
async fn execute_tools_node_runs_sequential_tool_and_appends_result_message() {
    ...
    flow.add_node("prepare_provider_request", PrepareProviderRequestNode)
        ...
        .unwrap()
        .edge("prepare_provider_request", "provider_stream")
        .unwrap()
...
async fn execute_tools_node_honors_after_hook_result_update() {
    ...
    agent.add_tool(AgentTool::new_text(
        ...
        "echo input",
        serde_json::json!({"type": "object"}),
        |_, _| async { Ok("original".to_string()) },
    ));
...
async fn decide_node_extracts_tool_calls_and_returns_tools_action() {
    ...
    assert_eq!(outcome.last_node.as_str(), "decide_after_assistant");
...
async fn decide_after_assistant_tool_use_without_calls_routes_to_tools() {
    ...
    flow.add_node("decide_after_assistant", DecideAfterAssistantNode)
        ...
        .add_node(
            "drain_queued_input",
            pi_agent_core::api::testing::DrainQueuedInputNode,
        )
        .unwrap()
        .edge_on(
            "decide_after_assistant",
