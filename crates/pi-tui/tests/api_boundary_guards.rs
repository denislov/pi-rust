...
fn root_public_modules_are_marked_migration_private() {
    ...
    let lib_source = fs::read_to_string(crate_root.join("src/lib.rs")).expect("read pi-tui lib.rs");
    ...
    assert!(
        violations.is_empty(),
        "pi_tui::api is the stable facade; root public modules must be explicitly migration-private:\n{}",
        violations.join("\n")
    );
}
