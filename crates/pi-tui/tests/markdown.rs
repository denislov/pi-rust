use pi_tui::{
    Color, Component, DefaultTextStyle, Markdown, MarkdownTheme, Style, color_enabled, paint_with,
    visible_width,
};
...
#[test]
fn renders_table_with_column_width_variance() {
    let mut md =
        Markdown::new("| Short | Very long column header that is wide |\n| --- | --- |\n| A | B |");
    let lines = md.render(80);
    let joined = lines.join("\n");

    assert!(joined.contains("Short"), "{joined}");
    assert!(
        joined.contains("Very long column header that is wide"),
        "{joined}"
    );

    // The wide column should have visible width >= the header text
    // Just check structure
    assert!(
        joined.contains("┌──"),
        "top border has wrong chars: {joined}"
    );
}
...
#[test]
fn renders_table_with_multi_col_wrapping() {
    ...
    // Content preserved
    // Content may be wrapped across lines, so check for words instead of phrases
    assert!(joined.contains("Command"), "{joined}");
    assert!(joined.contains("Description"), "{joined}");
    assert!(joined.contains("npm"), "npm* should appear: {joined}");
    assert!(
        joined.contains("install"),
        "install should appear: {joined}"
    );
...
#[test]
fn renders_table_without_extra_blank_line_at_end() {
    let mut md = Markdown::new("| Name |\n| --- |\n| Alice |");
    let lines = md.render(80);

    // The last line should contain the bottom border (└─┘), not be blank
    assert!(
...
fn table_with_inline_code() {
    let mut md = Markdown::new("| `code` | normal |\n| --- | --- |\n| data | more |");
    let lines = md.render(80);
    let joined = lines.join("\n");
    assert!(joined.contains("code"), "inline code content: {joined}");
    // code styling: reverse or yellow depending on theme
    assert!(joined.contains("normal"), "plain text: {joined}");
}

#[test]
fn table_in_document_with_heading_before_and_paragraph_after() {
    let mut md = Markdown::new("# Data\n\n| X | Y |\n| --- | --- |\n| 1 | 2 |\n\nEnd.");
    let lines = md.render(80);
    let joined = lines.join("\n");
    assert!(joined.contains("Data"), "heading: {joined}");
    assert!(joined.contains("End"), "trailing text: {joined}");
    assert!(joined.contains("┌"), "table start: {joined}");
    assert!(joined.contains("┘"), "table end: {joined}");
}

// ── DefaultTextStyle (pre-styled text / thinking traces) ─────────────
...
#[test]
fn partial_closing_fence_single_backtick_is_stripped() {
    let mut md = Markdown::new("```ts\nconst x = 1;\n`");
    let lines = md.render(80);
    let joined = lines.join("\n");
    assert!(
        !joined.contains("\n   `"),
        "should not show partial fence: {joined}"
    );
    assert!(
...
#[test]
fn partial_closing_fence_two_backticks_is_stripped() {
    let mut md = Markdown::new("```ts\nconst x = 1;\n``");
    let lines = md.render(80);
    let joined = lines.join("\n");
    assert!(
...
#[test]
fn partial_fence_tilde_is_stripped() {
    let mut md = Markdown::new("~~~\ncontent\n~~");
    let lines = md.render(80);
    let joined = lines.join("\n");
    ...
    // The partial fence ~~ should have been stripped; check that there
    // are only 3 lines (2 fence rows + 1 content line), not 4.
    // The code block has: opening fence ```, content, closing fence ```
    assert_eq!(
        lines.len(),
        3,
        "expected 3 lines (fence, content, fence), got {}: {:?}",
        lines.len(),
        lines
    );
}
...
#[test]
fn heading_followed_by_paragraph_has_one_blank_line() {
    let mut md = Markdown::new("# Hello\n\nThis is a paragraph");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    let hello_idx = plain.iter().position(|l| l.contains("Hello")).unwrap();
    let after_heading = &plain[hello_idx + 1..];
    let empty_count = after_heading.iter().position(|l| !l.is_empty()).unwrap();
    assert_eq!(
        empty_count, 1,
        "expected 1 blank line after heading, got {empty_count}: {plain:?}"
    );
}

#[test]
fn heading_without_source_blank_line_still_has_spacing() {
    // Even without blank line in source, heading and paragraph should have spacing
    let mut md = Markdown::new("# Hello\nThis is a paragraph");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    let hello_idx = plain.iter().position(|l| l.contains("Hello")).unwrap();
    ...
    assert_eq!(
        empty_count, 1,
        "expected 1 blank line after heading even without source blank: {plain:?}"
    );
}

#[test]
fn code_block_followed_by_paragraph_has_one_blank_line() {
    let mut md = Markdown::new("```js\nconst x = 1;\n```\n\nParagraph after");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    // Find the closing fence and check there's a blank before the paragraph
    let fence_close_idx = plain.iter().rposition(|l| l.contains("```")).unwrap();
    let after_code = &plain[fence_close_idx + 1..];
    // There should be exactly 1 blank line before the paragraph
    let empty_count = after_code.iter().position(|l| !l.is_empty()).unwrap();
    assert_eq!(
        empty_count, 1,
        "expected 1 blank line after code block: {plain:?}"
    );
    assert!(
        after_code[empty_count].contains("Paragraph after"),
        "paragraph should follow: {plain:?}"
    );
}

#[test]
fn blockquote_followed_by_paragraph_has_one_blank_line() {
    let mut md = Markdown::new("> A quote\n\nNext paragraph");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    // Find the last quote line ("│ A quote")
    let quote_idx = plain.iter().rposition(|l| l.contains("quote")).unwrap();
    let after_quote = &plain[quote_idx + 1..];
    let empty_count = after_quote.iter().position(|l| !l.is_empty()).unwrap();
    assert_eq!(
        empty_count, 1,
        "expected 1 blank line after blockquote: {plain:?}"
    );
}

#[test]
fn divider_followed_by_paragraph_has_one_blank_line() {
    let mut md = Markdown::new("---\n\nNext paragraph");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    // Find a line that is all hyphens (the divider renders as hyphens)
    let divider_idx = plain
        .iter()
        .position(|l| !l.is_empty() && l.chars().all(|c| c == '-'))
        .unwrap();
    let after_divider = &plain[divider_idx + 1..];
    let empty_count = after_divider.iter().position(|l| !l.is_empty()).unwrap();
    assert_eq!(
        empty_count, 1,
        "expected 1 blank line after divider: {plain:?}"
    );
}

#[test]
fn heading_as_last_block_has_no_trailing_blank_line() {
    let mut md = Markdown::new("# Hello");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    assert!(
...
#[test]
fn code_block_as_last_block_has_no_trailing_blank_line() {
    let mut md = Markdown::new("```\ncode\n```");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    assert!(
...
#[test]
fn blockquote_as_last_block_has_no_trailing_blank_line() {
    let mut md = Markdown::new("> A quote");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    // The last non-empty line should be the quote
    let last_content = plain.iter().rfind(|l| !l.trim().is_empty()).unwrap();
    assert!(
        last_content.contains("quote"),
        "last content should be quote: {plain:?}"
    );
}

#[test]
fn divider_as_last_block_has_no_trailing_blank_line() {
    let mut md = Markdown::new("---");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    let last_content = plain.iter().rfind(|l| !l.trim().is_empty()).unwrap();
    assert!(
        !last_content.is_empty() && last_content.chars().all(|c| c == '-'),
        "last content should be divider (hyphens): {plain:?}"
    );
}

#[test]
fn table_as_last_block_has_no_trailing_blank_line() {
    let mut md = Markdown::new("| A | B |\n| --- | --- |\n| 1 | 2 |");
    let lines = md.render(80);
    let plain = plain_lines(&lines);
    let last_content = plain.iter().rfind(|l| !l.trim().is_empty()).unwrap();
    assert!(
        ...
        "last content should be table bottom border: {plain:?}"
    );
}
...
#[test]
fn render_cache_updates_on_consecutive_calls() {
    let mut md = Markdown::new("A long text that will produce multiple lines when wrapped");
    // First render populates cache
    md.render(50);
    // Second render should hit cache (same text, same width)
    let cached = md.render(50);
    // Third render at different width bypasses cache and produces different output
    let different = md.render(20);
    assert_ne!(cached, different, "different widths should differ");
    // Fourth render at original width should return cached (same width, same content)
    let again = md.render(50);
    assert_eq!(cached, again, "same width should re-use cache");
}
