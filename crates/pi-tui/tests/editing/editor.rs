...
use pi_tui::api::component::{CURSOR_MARKER, Component, Editor, extract_cursor_marker};
...
fn editor_wraps_to_width_and_keeps_lines_bounded() {
    ...
    for line in editor.render(4) {
        assert!(pi_tui::api::render::visible_width(&line) <= 4);
    }
}
...
fn editor_treats_cursor_marker_as_zero_width_when_wrapping() {
    ...
    assert_eq!(pi_tui::api::render::visible_width(&lines[0]), 4);
}
