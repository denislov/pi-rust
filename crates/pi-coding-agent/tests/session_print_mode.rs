...
async fn enabled_session_with_name_uses_rust_native_log() {
    ...
    let _provider_guard =
        ProviderGuard::register(api, Arc::new(FauxProvider::simple_text("named")));
    let dir = tempfile::tempdir().unwrap();
...
async fn continues_most_recent_rust_native_session() {
    ...
    {
        let _first_provider_guard =
            ProviderGuard::register(api_first, Arc::new(FauxProvider::simple_text("stored")));
    ...
    let _second_provider_guard =
        ProviderGuard::register(api_second, Arc::new(FauxProvider::simple_text("continued")));
