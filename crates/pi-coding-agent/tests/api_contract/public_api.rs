...
use pi_coding_agent::api::event::{
    ...
    CodingAgentSubmittedEventDurability, CodingAgentTeamProductEvent, CodingAgentToolProductEvent,
...
use pi_coding_agent::api::operation::{
    ...
    CodingAgentPluginLoadOutcome, PendingDelegationConfirmation, PromptTurnMode, PromptTurnOptions,
...
use pi_coding_agent::api::runtime::{
    ...
    CodingAgentShutdownOutcome, CodingSessionError,
};
use pi_coding_agent::api::view::{
...
fn stable_api_signature_closure_is_importable() {
    ...
    for type_name in [
        ...
        name::<CodingAgentOperationTask>(),
        name::<CodingAgentCapabilityControl>(),
        name::<CodingAgentCapabilityRevocationOutcome>(),
        ...
        name::<CodingAgentOperationControl>(),
        name::<AgentProfile>(),
        name::<TeamProfile>(),
    ...
    let _product_event_operation_id = CodingAgentProductEvent::operation_id;
    let _product_event_stream_id = CodingAgentProductEvent::stream_id;
    let _product_event_parent_operation_id = CodingAgentProductEvent::parent_operation_id;
    let _product_event_root_operation_id = CodingAgentProductEvent::root_operation_id;
    let _product_event_session_id = CodingAgentProductEvent::session_id;
    let _product_event_delivery_class = CodingAgentProductEvent::delivery_class;
...
async fn outcome_acknowledgement_is_distinct_from_event_acknowledgement() {
    ...
    let acknowledgement = match submitted.status {
        pi_coding_agent::api::client::CodingAgentSubmittedOperationStatus::Terminal {
...
async fn client_connection_state_takeover_ack_and_drafts_are_generation_scoped() {
    ...
    first
        .set_prompt_draft(
            pi_coding_agent::api::client::CodingAgentDraftId("draft-1".into()),
            "hello",
        )
        .unwrap();
...
async fn detach_outcomes_and_lifecycle_rejection_paths_are_typed_and_preserve_state() {
    ...
    assert_eq!(
        first
            ...
            .unwrap_err(),
        pi_coding_agent::api::client::CodingAgentMutationRejection::Detached
    );
    assert_eq!(
        first
...
async fn detach_wakes_a_blocked_reconnect_receiver_without_leaking_an_event() {
    ...
    let connection = session
        ...
        .unwrap();
    let pi_coding_agent::api::client::CodingAgentReconnect::Replayed { mut receiver, .. } =
...
#[tokio::test]
async fn shutdown_drains_slow_reconnect_terminal_event_before_runtime_shutdown_and_close() {
    let api = "public-api-slow-reconnect-shutdown";
    ...
    let prompt = PromptTurnOptions::from_prompt_run_options(PromptRunOptions {
        ...
        settings: None,
        invocation: PromptInvocation::Text("slow reconnect".into()),
    });
    let mut session = CodingAgentSession::non_persistent(CodingAgentSessionOptions::new())
        .await
        .unwrap();
    ...
    let connection = session
        .connect(CodingAgentClientId::new("slow-reconnect-client"))
        .unwrap();
    let pi_coding_agent::api::client::CodingAgentReconnect::Replayed { mut receiver, .. } =
        connection.reconnect(0).unwrap()
    else {
    ...
    loop {
        ...
        if matches!(
            event,
            pi_coding_agent::api::client::CodingAgentReconnectDelivery::Event(ref event)
                if matches!(
                    event.event(),
        ...
        assert!(!matches!(
            event,
            pi_coding_agent::api::client::CodingAgentReconnectDelivery::Event(event)
                if matches!(
                    event.event(),
    ...
    let shutdown = receiver.recv().await.unwrap();
    assert!(matches!(
        shutdown,
        pi_coding_agent::api::client::CodingAgentReconnectDelivery::Event(event)
            if matches!(
...
async fn client_connection_replays_unacknowledged_delivery_and_ack_is_explicit() {
    ...
    match recovery {
        pi_coding_agent::api::client::CodingAgentReconnect::Replayed { events, cursor, .. } => {
...
async fn reconnect_from_cursor_validates_stream_and_snapshot_major() {
    ...
    let other_session = CodingAgentSession::non_persistent(CodingAgentSessionOptions::new())
        .await
        .unwrap();
    let other_connection = other_session
        .connect(CodingAgentClientId::new("other-runtime"))
        .unwrap();
    assert_ne!(
        other_connection.state().unwrap().cursor.stream_id,
        cursor.stream_id
    );
    assert_eq!(
        other_connection
            .reconnect_from_cursor(&cursor)
            .unwrap_err()
            .code(),
        "input"
    );
    assert!(matches!(
        ...
        Ok(pi_coding_agent::api::client::CodingAgentReconnect::Replayed { .. })
    ));
...
async fn submission_lease_drop_preserves_draft_and_releases_exclusivity() {
    ...
    connection
        .set_prompt_draft(
            pi_coding_agent::api::client::CodingAgentDraftId("draft-lease".into()),
            "tracked prompt",
        )
        .unwrap();
...
async fn submission_lease_canonical_run_clears_draft_and_records_terminal() {
    ...
    let connection = session
        ...
        .unwrap();
    let draft_id = pi_coding_agent::api::client::CodingAgentDraftId("draft-run".into());
    connection
    ...
    let pi_coding_agent::api::client::CodingAgentReconnect::Replayed { events, cursor, .. } =
        connection.reconnect(0).unwrap()
    else {
...
fn client_reconnect_source_uses_only_the_atomic_replay_live_boundary() {
    let connection = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/runtime/client/projection.rs"),
    )
    .unwrap();
    let coordinator = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/runtime/snapshot.rs"),
    )
    .unwrap();
...
fn snapshot_topology_has_one_registry_and_zero_authority_client_facade() {
    ...
    let coordinator = fs::read_to_string(crate_root.join("src/runtime/snapshot.rs")).unwrap();
    let client_service =
        fs::read_to_string(crate_root.join("src/runtime/client/service.rs")).unwrap();
    let event_service = fs::read_to_string(crate_root.join("src/services/event.rs")).unwrap();
...
fn snapshot_writers_1_startup_drain_releases_marker_lock_before_projection() {
    let owner = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/runtime/facade/connection.rs"),
    )
    .unwrap();
...
fn snapshot_writers_2_operation_guard_releases_owner_lock_before_projection_clear() {
    let operation_control = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/runtime/control.rs"),
    )
    .unwrap();
    let guard_drop = operation_control
        .find("impl Drop for OperationGuard")
        .unwrap();
    let guard_drop = &operation_control[guard_drop..];
    let recompute = guard_drop
        .find("let current_primary = shared.activity().primary();")
        .unwrap();
    let drop_lock = guard_drop.find("drop(shared);").unwrap();
    let clear_cancellation = guard_drop
        .find("clear_operation_cancellation_if(&self.operation_id)")
        .unwrap();
    let project_remaining = guard_drop
        .find("set_active_operation(current_primary)")
        .unwrap();
    assert!(recompute < drop_lock);
    assert!(drop_lock < clear_cancellation && clear_cancellation < project_remaining);
}
...
fn snapshot_writers_4_navigation_refreshes_projection_before_publication() {
    let crate_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let owner = fs::read_to_string(crate_root.join("src/runtime/dispatch.rs")).unwrap();
    let fork_arm = &owner[owner
...
fn snapshot_writers_5_event_commit_releases_coordinator_before_broadcast() {
    let event_service = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/services/event.rs"),
    )
    .unwrap();
...
fn coding_session_product_event_subscription_public_facade_is_importable() {
    ...
    let contract_types = [
        ...
        std::any::type_name::<CodingAgentSessionWriteFailureStatus>(),
