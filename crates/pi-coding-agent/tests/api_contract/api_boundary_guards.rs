...
const FAIL_FIXTURES: [CompileFixture; 15] = [
    ...
    CompileFixture {
        ...
        access_path: "root",
        source: "args_root.rs",
        expected: unresolved(22, 26, "args", "pi_coding_agent"),
    },
    CompileFixture {
        category: "legacy-root-error-module",
        access_path: "root",
        source: "error_root.rs",
        expected: unresolved(22, 27, "error", "pi_coding_agent"),
    },
    CompileFixture {
        category: "legacy-root-prompt-options-module",
        access_path: "root",
        source: "prompt_options_root.rs",
        expected: unresolved(22, 36, "prompt_options", "pi_coding_agent"),
    },
];
...
fn api_is_the_only_public_root_module() {
    ...
    assert!(
        violations.is_empty(),
        "pi_coding_agent::api must remain the only public root module:\n{}",
        violations.join("\n")
    );
}
...
#[test]
fn cli_facade_has_only_scripting_scenarios_and_no_flat_exports() {
    let crate_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let lib_source = fs::read_to_string(crate_root.join("src/lib.rs"))
        .expect("pi-coding-agent lib.rs should be readable");
    let cli_module =
    ...
    for (index, line) in cli_module.lines().enumerate() {
        let trimmed = line.trim();
        if depth == 0 {
            if let Some(rest) = trimmed.strip_prefix("pub mod ") {
                if let Some(name) = rest.split_whitespace().next() {
    ...
    assert_eq!(actual, expected, "CLI scripting scenario set drifted");
    assert!(
        flat_exports.is_empty(),
        "CLI facade must not regain an unrestricted flat surface:\n{}",
        flat_exports.join("\n")
    );
}
...
fn implementation_modules_stay_private_and_retired_roots_stay_absent() {
    ...
    assert!(app_source.contains("pub(crate) mod cli;"));
    assert!(app_source.contains("pub(crate) mod shutdown;"));
    assert!(!app_source.contains("pub mod cli;"));
    assert!(!app_source.contains("pub mod shutdown;"));

    let cli_source = fs::read_to_string(crate_root.join("src/app/cli/mod.rs"))
        .expect("CLI owner should be readable");
    for owner in [
        "args",
        "error",
        "input",
        ...
        assert!(
            cli_source.contains(&format!("pub(crate) mod {owner};")),
            "CLI implementation owner `{owner}` must remain crate-private"
        );
        assert!(
            !cli_source.contains(&format!("pub mod {owner};")),
            "CLI implementation owner `{owner}` must not become public"
        );
    }

    let protocol_source = fs::read_to_string(crate_root.join("src/protocol/mod.rs"))
        .expect("protocol module should be readable");
    for retired in ["events", "json_mode", "rpc"] {
        assert!(
            !protocol_source.contains(&format!("mod {retired};"))
                && !protocol_source.contains(&format!("pub mod {retired};")),
            "retired protocol adapter module `{retired}` must stay absent"
        );
    }

    let adapters_source = fs::read_to_string(crate_root.join("src/adapters/mod.rs"))
        .expect("adapter module should be readable");
    for owner in ["events", "interactive", "json", "print", "rpc"] {
        assert!(
            adapters_source.contains(&format!("pub(crate) mod {owner};")),
            "adapter owner `{owner}` must remain crate-private"
        );
        assert!(
            !adapters_source.contains(&format!("pub mod {owner};")),
            "adapter owner `{owner}` must not become public"
        );
    }
...
fn root_reexports_are_removed_after_breaking_facade_migration() {
    ...
    assert!(
        violations.is_empty(),
        "root reexports must stay removed; stable users import a categorized pi_coding_agent::api path:\n{}",
        violations.join("\n")
    );
}
