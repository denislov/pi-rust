...
use pi_ai::api::conversation::{ContentBlock, StopReason, Usage};
...
use pi_coding_agent::api::protocol::{
    CodingProtocolEventAdapter, CompactionReason, ProtocolEvent, RpcDetachLifecycleEvent,
    RpcDetachRequest, RpcDetachResponse, RpcDetachStatus, RpcShutdownLifecycleEvent,
    RpcShutdownRequest, RpcShutdownResponse, RpcShutdownStatus,
};
...
fn coding_event_adapter_maps_session_write_failure_state() {
    ...
    assert_eq!(
        adapter.push_product_event(&event),
        vec![ProtocolEvent::SessionWriteFailed {
            operation_id: "op-write".into(),
...
fn product_event_adapter_reads_the_owned_typed_payload() {
    let source = include_str!("../../src/adapters/events.rs");
    let rpc_source = include_str!("../../src/adapters/rpc/events.rs");
...
fn coding_event_adapter_maps_prompt_failure_with_provider() {
    ...
    let events = adapter.push_product_event(&product_event(CodingAgentProductEventKind::Workflow(
        CodingAgentWorkflowProductEvent::PromptFailed {
            operation_id: "op_1".into(),
            error: product_error(
                pi_coding_agent::api::runtime::CodingSessionError::Provider {
