...
use super::session_service::SessionPersistence;

#[derive(Debug, Default)]
pub(crate) struct DelegationConfirmationService;

impl DelegationConfirmationService {
    ...
    pub(crate) fn approve_pending(
        ...
        let pending = self.active_pending(queue, operation_id, tool_call_id, now)?;
    ...
    pub(crate) fn adopt_pending(
        ...
        for pending in pending_confirmations {
            self.queue_pending(persistence, queue, event_service, pending, false)?;
        }
        Ok(())
    }
