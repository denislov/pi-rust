...
pub fn select_model(
    ...
    if let Some(models) = args.models.as_deref() {
        let rotation = crate::app::cli::models::parse_model_rotation(models)?;
    ...
    if let Some(model_id) = &args.model {
        let model = pi_ai::api::model::lookup_model(model_id)
    ...
    pi_ai::api::model::lookup_model(DEFAULT_MODEL_ID)
...
pub(crate) fn build_agent_config_with_auth_diagnostics(
    ...
    if let Some(settings) = settings {
        config.steering_mode = settings
            .steering_mode
            .parse()
            .unwrap_or(pi_agent_core::api::agent::QueueMode::OneAtATime);
