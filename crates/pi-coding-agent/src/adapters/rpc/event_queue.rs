...
mod tests {
    ...
    async fn rpc_product_event_queue_is_bounded_and_ordered() {
        ...
        assert!(matches!(
            ...
            RpcQueuedProductEvent::Event(product_event) if product_event.sequence() == 1
        ));
        assert!(matches!(
            receiver.recv().await.unwrap(),
            RpcQueuedProductEvent::Event(product_event) if product_event.sequence() == 2
        ));
    }
