...
use pi_tui::api::theme::{TuiTheme, dark_theme, light_theme};
...
use crate::adapters::interactive::session_actions::{SessionChoice, collect_session_choices};
#[cfg(test)]
...
use crate::app::cli::request::{
    configured_model_choices, resolve_cli_context_from_options, resolve_profile_registry,
    rotation_model_choices,
};
use crate::app::session::ResolvedSessionTarget;
use crate::app::shutdown::shutdown_session;
use crate::resources;
use crate::runtime::facade::{ProfileId, ProfileRegistry};
...
pub async fn run_interactive_mode(parsed: CliArgs, options: CliRunOptions) -> CliOutput {
    ...
    match run_interactive_loop_with_input(parsed, options, terminal, InputPump::from_stdin).await {
        Ok(mut result) => {
            let shutdown_error = shutdown_session(result.coding_session.take()).await.err();
...
pub(super) struct PromptContext {
    ...
    pub(super) tools: Vec<pi_agent_core::api::tool::AgentTool>,
    ...
    pub(super) thinking_level: Option<pi_agent_core::api::agent::ThinkingLevel>,
    pub(super) tool_execution: Option<pi_agent_core::api::tool::ToolExecutionMode>,
...
pub(super) fn build_prompt_context(
    ...
    let resolved = resolve_cli_context_from_options(parsed.clone(), options)?;
    let diagnostic_text = crate::app::cli::request::render_diagnostics(&resolved.diagnostics);
    ...
    let themes_dir = resolved.config_paths.global_dir.join("themes");
...
mod tests {
    ...
    fn opts(width: usize, max_tool_result_lines: usize) -> TranscriptRenderOptions<'static> {
        TranscriptRenderOptions {
            ...
            color: false,
            markdown_theme: pi_tui::api::theme::MarkdownTheme::default(),
    ...
    fn ctrl_p_event(shift: bool) -> InputEvent {
        let mut modifiers = pi_tui::api::input::KeyModifiers::CTRL;
        if shift {
            modifiers.insert(pi_tui::api::input::KeyModifiers::SHIFT);
        }
        InputEvent::Key(pi_tui::api::input::KeyEvent {
            key: pi_tui::api::input::Key::Char(if shift { "P".into() } else { "p".into() }),
            modifiers,
    ...
    fn build_prompt_context_applies_selected_theme_to_editor_borders() {
        ...
        assert_eq!(
            ...
            pi_tui::api::render::Color::Rgb(0x21, 0x11, 0x44)
        );
        assert_eq!(
            ctx.theme.editor.menu_border.fg,
            pi_tui::api::render::Color::Rgb(0x12, 0x34, 0xaa)
        );
    }
    ...
    fn render_transcript_lines_colors_error_item_red_bold() {
        use pi_tui::api::render::{ERROR, paint_with};
    ...
    fn footer_shows_thinking_level_for_reasoning_model() {
        ...
        root.thinking_level = pi_agent_core::api::agent::ThinkingLevel::High;
        let footer = root.footer(80).join("\n");
        assert!(footer.contains("m • high"), "{footer}");

        root.thinking_level = pi_agent_core::api::agent::ThinkingLevel::Off;
        let footer = root.footer(80).join("\n");
    ...
    fn editor_border_uses_active_theme_style_in_normal_input_state() {
        let theme = pi_tui::api::theme::TuiTheme::custom(
            "custom",
            pi_tui::api::theme::ThemePalette {
                accent: pi_tui::api::render::Color::Cyan,
                muted: pi_tui::api::render::Color::Ansi256(244),
                text: pi_tui::api::render::Color::White,
                background: pi_tui::api::render::Color::Default,
                error: pi_tui::api::render::Color::Red,
                success: pi_tui::api::render::Color::Green,
                warning: pi_tui::api::render::Color::Yellow,
                path: pi_tui::api::render::Color::Cyan,
                input_border: pi_tui::api::render::Color::Rgb(10, 20, 30),
                menu_border: pi_tui::api::render::Color::Rgb(40, 50, 60),
            },
        );
        let mut root = InteractiveRoot::new(
            PathBuf::from("."),
        ...
        assert_eq!(
            ...
            pi_tui::api::render::Color::Rgb(10, 20, 30)
        );
    ...
    fn editor_border_reflects_thinking_level_from_resolved_theme() {
        ...
        assert_eq!(
            root.editor_border_style().fg,
            pi_tui::api::render::Color::Rgb(0xb2, 0x94, 0xbb)
        );
        ...
        assert_eq!(
            root.editor_border_style().fg,
            pi_tui::api::render::Color::Rgb(0x50, 0x50, 0x50)
        );
    }
    ...
    fn settings_menu_thinking_level_cycles_and_reports_update() {
        ...
        assert_eq!(
            root.thinking_level,
            pi_agent_core::api::agent::ThinkingLevel::Off
        );
        ...
        assert_eq!(
            root.thinking_level,
            pi_agent_core::api::agent::ThinkingLevel::Minimal
        );
        assert_eq!(
            root.take_selected_thinking_level(),
            Some(pi_agent_core::api::agent::ThinkingLevel::Minimal)
        );
        let updated = root
            .take_settings_update()
    ...
    fn ctrl_p_cycles_models_from_rotation() {
        let rotation = vec![
            pi_ai::api::model::lookup_model("claude-haiku-4-5").unwrap(),
            pi_ai::api::model::lookup_model("gpt-5").unwrap(),
            pi_ai::api::model::lookup_model("gpt-5-mini").unwrap(),
        ];
    ...
    fn model_command_accepts_thinking_suffix() {
        ...
        assert_eq!(
            root.take_selected_thinking_level(),
            Some(pi_agent_core::api::agent::ThinkingLevel::High)
        );
    }
    ...
    fn expand_skill_command_expands_to_xml_block() {
        let skills = vec![pi_agent_core::api::resources::Skill {
            name: "rust".into(),
        ...
        let result = crate::adapters::interactive::commands::expand_skill_command(
    ...
    fn expand_skill_command_non_skill_passes_through() {
        let result = crate::adapters::interactive::commands::expand_skill_command("/help", &[]);
    ...
    fn expand_prompt_template_substitutes_args() {
        let templates = vec![pi_agent_core::api::resources::PromptTemplate {
            name: "review".into(),
        ...
        let result = crate::adapters::interactive::commands::expand_prompt_template(
            "/review code tests",
            &templates,
        );
    ...
    fn expand_prompt_template_unknown_passes_through() {
        let result =
            crate::adapters::interactive::commands::expand_prompt_template("/unknown blah", &[]);
    ...
    fn expand_prompt_template_non_slash_passes_through() {
        let result =
            crate::adapters::interactive::commands::expand_prompt_template("just text", &[]);
    ...
    fn handle_unknown_slash_command_with_template_expands_and_submits() {
        ...
        root.prompt_templates = vec![pi_agent_core::api::resources::PromptTemplate {
            name: "review".into(),
            description: "Review".into(),
    ...
    fn handle_unknown_slash_command_with_skill_expands_and_submits() {
        ...
        root.skills = vec![pi_agent_core::api::resources::Skill {
            name: "rust".into(),
            description: "Rust".into(),
            location: "/skills/rust/SKILL.md".into(),
            content: "Rust programming guide".into(),
...
"#,
    ...
    fn fork_command_sets_pending_request_for_rust_native_session() {
        ...
        assert_eq!(
            root.take_action(),
            crate::adapters::interactive::root::InteractiveAction::Fork
        );
        let request = root
    ...
    async fn real_stdin_reader_is_created_after_terminal_start() {
        ...
        impl Terminal for OrderingTerminal {
            fn size(&self) -> pi_tui::api::terminal::TerminalSize {
                pi_tui::api::terminal::TerminalSize {
                    columns: 80,
                    rows: 24,
                }
            }
...
pub mod test_harness {
    ...
    pub async fn run_scripted_interactive_with_provider_chunks(
        provider: Arc<dyn pi_ai::api::provider::ApiProvider>,
    ...
    pub async fn run_scripted_idle_interactive_with_delays(
        ...
        let owned_steps = steps
            ...
            .collect::<Vec<_>>();
        let clock = crate::adapters::interactive::r#loop::ManualInteractiveClock::new(
        ...
        let run = crate::adapters::interactive::r#loop::run_interactive_loop_with_clock(
            parsed,
            options,
            VirtualTerminal::new(columns, rows),
            &mut input,
