...
use pi_agent_core::api::transcript::{SessionTreeNode, create_timestamp};
#[cfg(test)]
...
use crate::app::session::{
    ResolvedSessionTarget, clone_interactive_session, export_interactive_session_html,
    hydrate_interactive_session_target, interactive_session_tree,
    list_interactive_session_hydrations,
};
use crate::runtime::facade::{CodingAgentSessionHydration, CodingAgentSessionTranscriptItem};
...
pub(super) fn hydrate_existing_session_target(
    ...
    Ok(hydrate_interactive_session_target(session_options, target)?
...
pub(super) fn clone_rust_native_choice(
    ...
    clone_interactive_session(&choice.path, Path::new(&choice.cwd))
        .map(hydrated_session_from_rust_native)
...
pub(super) fn rust_native_tree_for_choice(
    ...
    let tree = interactive_session_tree(&choice.path, Path::new(&choice.cwd))?;
...
pub(super) fn export_rust_native_choice(
    ...
    export_interactive_session_html(&choice.path, Path::new(&choice.cwd), &path)
...
pub(super) fn collect_session_choices(session: &Option<SessionRunOptions>) -> Vec<SessionChoice> {
    let mut choices = list_interactive_session_hydrations(session)
        .unwrap_or_default()
        .iter()
        .map(session_choice_from_rust_native)
        .collect::<Vec<_>>();
