...
use crate::app::cli::request::resolve_provider_api_key;
...
fn handle_input_event<T: Terminal>(
    ...
    if let Some(model) = selected_model {
        ...
        let diagnostic_text = crate::app::cli::request::render_diagnostics(&diagnostics);
    ...
    if let Some(settings) = settings_update {
        ...
        if let Ok(root) = root_mut(tui, root_id) {
            ...
            let _diagnostics = persist_global_settings(&cwd, delta);
        }
    }
    ...
    match action {
        ...
        InteractiveAction::NewSession => {
            if prompt_context.session.as_ref().is_some_and(|session| {
                matches!(session.mode, crate::app::bootstrap::SessionMode::Enabled)
            }) {
                *coding_session = None;
...
mod tests {
    ...
    fn prompt_event_with_sequence(
        ...
        PromptTaskEvent::Coding(ProductEvent::from_draft_for_tests(
            sequence,
    ...
    fn base_assistant_delta() -> PromptStreamEvent {
    ...
    fn tool_started() -> PromptStreamEvent {
    ...
    fn prompt_block_events_request_forced_render() {
        let cases = vec![
            ...
            (
                vec![tool_started()],
                PromptStreamEvent::Tool(ToolEvent::Updated {
                    operation_id: "op_1".to_string(),
                    turn_id: "turn_1".to_string(),
                    tool_call_id: "tool_1".to_string(),
                    name: "read".to_string(),
            ...
            (
                vec![tool_started()],
                PromptStreamEvent::Tool(ToolEvent::Failed {
                    operation_id: "op_1".to_string(),
                    turn_id: "turn_1".to_string(),
                    tool_call_id: "tool_1".to_string(),
                    name: "read".to_string(),
            ...
            (
                ...
                PromptStreamEvent::Delegation(DelegationEvent::Requested {
                    context: DelegationEventContext {
                        ...
                        turn_id: "turn_1".to_string(),
                        tool_call_id: "tool_1".to_string(),
                        requesting_profile_id: crate::runtime::facade::ProfileId::from("planner"),
                        target_kind: crate::runtime::facade::ProfileKind::Agent,
                        target_id: crate::runtime::facade::ProfileId::from("coder"),
                        task: "help".to_string(),
                    },
                }),
            ),
    ...
    async fn real_fork_failure_preserves_source_owner_subscriber_and_target_through_prompt_task_done()
     {
        ...
        while let Some(event) = source_receiver.try_recv().unwrap() {
            if matches!(
                event.event(),
                CodingAgentProductEventKind::Profile(crate::runtime::facade::CodingAgentProfileProductEvent::DefaultChanged { profile_id })
    ...
    async fn fork_completion_replaces_the_prompt_session_target() {
        ...
        finish_prompt(
            ...
            PromptTaskCompletion::Completed(PromptTaskResult::ForkSession(
                crate::adapters::interactive::prompt_task::ForkSessionTaskResult {
                    session,
