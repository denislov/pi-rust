...
2. **M13 周边能力** — JSONL 导入、完整 HTML 导出 parity、gist 分享、package manager（`/copy` 与 `/export` 基础导出已落地）。
3. **P0 修复** — Google Vertex provider（models_generated.rs 有引用但无实现）。
4. **P3 补齐** — provider-internal hook 覆盖、交互业务选择器、外设类 slash command 后端等 polish/产品化缺口。
