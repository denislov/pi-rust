...
> 健康度：截至 2026-06-20 核对工作区编译通过、`cargo fmt/check --workspace` 全绿。
> `cargo test --workspace` 有 2 个已知失败：`env_beats_auth_file`（环境相关）、
> `build_prompt_context_applies_selected_theme_to_editor_borders`（待修复）。
...
2. **M13 周边能力** — HTML 导出、JSONL 导入/导出、gist 分享、`/copy` 剪贴板、package manager、`--list-models`。
3. **P0 修复** — Google Vertex provider（models_generated.rs 有引用但无实现） + 2 个失败测试。
4. **P3 补齐** — `transformContext`/`agentLoopContinue`/流式 bash/diff 编辑/pluggable operations 等 agent-core 和 coding-agent 功能缺口。
