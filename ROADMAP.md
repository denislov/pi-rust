...
|---|---:|---:|---:|---|
| `pi-ai` | 30,553 | 23,669 | ~75% | ✅ M2，广度待补（[M8](docs/roadmap/M8-provider-breadth.md)） |
| `pi-agent-core` | 8,067 | 4,351 | 运行时内核完整 | ✅ M4，harness 待补（[M9](docs/roadmap/M9-agent-harness.md)） |
| `pi-coding-agent` | 48,013 | 6,673 | ~15% | 引擎在，生态待补（M7/M10/M11/M13） |
| `pi-tui` | 11,696 | 4,277 | core ~80% | ✅ M6 + TUI-8 部分，组件/主题待补（[M11](docs/roadmap/M11-interactive-ux.md)） |
...
> 健康度：截至 2026-06-19 核对工作区编译通过、`cargo fmt/test/check --workspace` 全绿（511 测试）。
> 如需最新数请复跑 `cargo test --workspace`。`docs/superpowers/plans/*.md` 复选框**不能**作完成信号。

**已完成项 + 本轮审阅修正项** → [docs/roadmap/done.md](docs/roadmap/done.md)
（修正：`pi-agent-core` 的并行执行/`ToolExecutionMode`/skills/prompt templates/thinking level 实际**已实现**。）
...
## 4. 建议近期下一步
1. **M7 配置+认证基座**——核心地基，解锁多 provider 实际可用。
2. **M9 agent-core harness**——不依赖认证，可与 M7/M8 并行推进。
3. **M8 provider 广度**——先 REST（Mistral/Azure），再 Bedrock/Codex。
4. 之后按依赖推进 M10 → M11 → M12 → M13。

> 每个里程碑落地前，按 brainstorming → spec（`docs/superpowers/specs/`）→ plan（`docs/superpowers/plans/`）流程细化。
