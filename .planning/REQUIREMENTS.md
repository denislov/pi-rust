...
- [ ] **FACADE-01**: First-party callers can obtain the complete stable `CodingAgentOperation`, outcome, and required support types through `pi_coding_agent::api`
...
- [ ] **FACADE-04**: Internal operations, dispatch metadata, plugin load options, services, and Flow nodes are not exposed through the stable API
...
- [ ] **RPC-01**: RPC prompt, agent, team, and delegation-approval background tasks execute through canonical operations
...
- [ ] **RPC-03**: The RPC migration preserves existing `tokio::select!` control handling, event forwarding, response shapes, and error protocol
- [ ] **RPC-04**: RPC production code contains no replaced broad workflow calls or local deprecation suppressions

### Interactive

- [ ] **INTER-01**: Interactive prompt, agent, team, compaction, self-healing, plugin, and branch-summary background work executes through canonical operations
- [ ] **INTER-02**: Interactive profile mutation and delegation rejection execute through canonical operations
- [ ] **INTER-03**: Session fork and navigation use canonical operations and refresh snapshots and projections after transitions
- [ ] **INTER-04**: The interactive migration preserves event/control multiplexing, subscriber continuity, product-event sequencing, and UI behavior
- [ ] **INTER-05**: Interactive production code contains no replaced broad workflow calls or local deprecation suppressions
