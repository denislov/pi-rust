...
- [ ] **EVENT-01**: Public consumers can inspect product-event kind through a stable typed enum rather than string-only family/kind fields.
- [ ] **EVENT-02**: Public product events expose stable sequence, operation identity, terminal status, durability, and documented payload semantics where the underlying event provides them.
