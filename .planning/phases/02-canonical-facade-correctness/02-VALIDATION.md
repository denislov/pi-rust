...
| 02-03-03 | 03 | 2 | FACADE-01, FACADE-02, FACADE-03, FACADE-04, FACADE-05 | T-02-01 through T-02-08 | Final tree passes focused, crate, workspace, executable failure-control/facade boundary tests, the path-safe adapter scope audit, formatting, and diff gates | phase closure | `cargo fmt --check && cargo test -p pi-coding-agent && cargo check -p pi-coding-agent && cargo test --workspace && cargo check --workspace && cargo test -p pi-coding-agent --test public_api --test api_boundary_guards --test product_runtime_boundary_guards -- --nocapture && sh -c 'for path in crates/pi-coding-agent/src/protocol crates/pi-coding-agent/src/print_mode.rs crates/pi-coding-agent/src/interactive; do test -e "$path" || exit 2; done; rg -n "CodingAgentOperation::" crates/pi-coding-agent/src/protocol crates/pi-coding-agent/src/print_mode.rs crates/pi-coding-agent/src/interactive; rg_status=$?; test "$rg_status" -eq 1' && git diff --check` | ✅ extend | ⬜ pending |
...
- [ ] Extend `product_runtime_boundary_guards.rs` with exact test-only ownership checks for every session-store failure-control definition/use and with a Phase 1 inventory guard against new workflow wrappers, aliases, traits, modules, or a second facade around `CodingAgentSession::run`.
