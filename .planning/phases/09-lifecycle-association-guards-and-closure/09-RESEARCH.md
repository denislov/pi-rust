...
Three live gaps drive implementation. First, `CodingAgentReconnectReceiver::recv()` currently waits only on a broadcast receiver, so marking a generation stale cannot wake a blocked receiver; detach/shutdown needs a lifecycle notification branch and every delivery must revalidate lifecycle state before projection. Second, client submission provenance is Prompt-only: `submission_fingerprint()` returns `None` for 14 variants and `prepare_submission()` rejects them, while the locked Phase 9 contract requires OutcomeOnly submitted terminal state. The submission lease must become operation-generic, with Prompt draft validation/clearing retained as a Prompt-specific admission rule. Third, Compact cancellation is not currently a canonical production path: `ManualCompactionFlow::run_with_options` accepts `FlowRunOptions.cancel`, but `FlowService::run_manual_compaction_graph` calls the default `run`, `ManualCompactionOptions` carries no token, and `manual_compaction_flow::flow_error` maps `FlowError::Cancelled` to generic `CodingSessionError::Flow`. Phase 9 must bind the admission operation id and monotonic internal generation to a private active identity, carry its Compact-only token on `OperationPermit` into `FlowRunOptions`, and expose a cloneable crate-private `CompactCancellationHandle` that can request cancellation by exact active Compact id while the unique session owner is moved. This is unconditional production-shaped internal plumbing, not a public connection control or test-only branch. [VERIFIED: `mod.rs:343-357,1226-1315`; `manual_compaction_flow.rs:97-147,440-477,522-526`; `flow_service.rs:207-221`; `operation_control.rs:92-226`; `intent_router.rs:37-104,166-188`; `error.rs:1-54`]
...
| `Compact` | `Compact` | `TerminalAssociated` | Success emits same-id `Session.CompactionCompleted -> Compact` and compatibility `Workflow.PromptCompleted -> Prompt`; association-relative cardinality counts only the descriptor-permitted id plus admitted Compact kind, so `PromptCompleted` is excluded. Provider failure and canonical operation-scoped Flow cancellation both map to `PromptTurnOutcome::Failed` and same-id `Workflow.PromptFailed`; `CodingSessionError::Cancelled` distinguishes cancellation and there is no aborted compatibility branch. Current source does not yet provide a reachable caller while `run(&mut self)` owns the session, so Plan 04 owns a pre-transfer cloneable crate-private `CompactCancellationHandle`, exact active-identity validation, permit token wiring, and colocated canonical-run coverage before asserting cancellation association. [VERIFIED: `mod.rs:343-357,1226-1315`; `manual_compaction_service.rs:20-72`; `manual_compaction_flow.rs:440-477,522-526`; `flow_service.rs:207-221`; `event.rs:89-93,108-110`] |
...
`CodingAgentSession::run` takes `&mut self`, and adapters move the owner into a background task and receive it back in the task result. Therefore Phase B/final publication occurs after that owner returns, while Phase A is requested concurrently through an opaque `CodingAgentRuntimeShutdownHandle` captured before owner transfer. The handle clones only the existing `Arc<SnapshotCoordinator>`, can request the shared runtime transition and wake lifecycle waiters, and cannot dispatch operations, select clients/generations, publish the final event, or complete shutdown. Every sync and async `run` admission checks this shared lifecycle state. `OperationGuard::drop` already calls `SnapshotCoordinator::set_active_operation(None)` and is extended to notify drain waiters. Do not create a second owner or ordinary dispatcher to work around Rust ownership. [VERIFIED: `mod.rs:343-357`; `operation_control.rs:92-140,217-226`; `protocol/rpc/state.rs:59-80`; interactive owner-restoration tests]
...
Add a separate typed `acknowledge_outcome(CodingAgentOutcomeAcknowledgementId)` connection method. The id is an opaque public newtype minted through the session's existing deterministic id-generation path at admitted finalization and carried in submitted state; its inner construction stays private and contains no generation, receipt signature, or durability encoding. It is operation-scoped, generation/client scoped at acknowledgement, and only clears a matching OutcomeOnly terminal anchor. [VERIFIED: locked D-09/D-14]
...
For `TerminalAssociated`, return the exact `ProductEvent` from each root-emission helper and propagate it directly into canonical finalization. Validate descriptor-permitted variant, operation id, admitted kind, outcome branch, and one-time cardinality; do not add a coordinator terminal index or scan retained history. Compact success emits both `Session.CompactionCompleted` and compatibility `Workflow.PromptCompleted`; only the first matches the admitted Compact descriptor, while the latter remains Prompt-kind and is excluded from the Compact root count. Compact failure and cancellation both retain the existing same-id `Workflow.PromptFailed` plus `PromptTurnOutcome::Failed`; normalize that exact failed branch to admitted kind Compact while using the typed error state to distinguish cancellation. For OutcomeOnly, finalize directly from the typed `CodingAgentOperationOutcome`/error with the outcome anchor. [VERIFIED: `event_service.rs:278-296,418-492`; `manual_compaction_service.rs:39-72`; `flow_service.rs:214-221`; bounded retention at `event_service.rs:256-276`]
...
- each of the five TerminalAssociated operations has success/failure/cancellation coverage where the workflow supports each state, and asserts exactly one matching root terminal event before `run` returns; Compact cancellation reachability is a colocated lib test because its request handle is intentionally crate-private, while external association tests cover only public-observable branches;
...
Compact keeps its current event behavior: success emits same-id `Session.CompactionCompleted` and `Workflow.PromptCompleted`; exact association counts only `CompactionCompleted` because descriptor matching requires both operation id and admitted Compact root kind. Provider failure and operation-scoped Flow cancellation both produce `PromptTurnOutcome::Failed` and same-id `Workflow.PromptFailed`; cancellation remains distinguishable through `CodingSessionError::Cancelled` rather than an aborted compatibility event. The reachable canonical path uses a cloneable crate-private `CompactCancellationHandle` captured from `CodingAgentSession` before owner transfer. `OperationControl::begin` records the admitted operation id, kind, monotonic generation, and Compact-only token; `OperationGuard::drop` clears only its generation; `CompactCancellationHandle::cancel(operation_id)` cancels only the exact current Compact identity and returns typed rejection for no-active, non-Compact, or stale/mismatched identities. `OperationPermit` carries the token through `ManualCompactionOptions` into `ManualCompactionFlow::run_with_options`/`FlowRunOptions.cancel`, and `FlowError::Cancelled` maps explicitly. Because this seam is crate-private, the deterministic canonical cancellation case belongs in `coding_session/mod.rs`'s colocated `#[cfg(test)]` module: retain the generalized submission connection and handle before moving the session, observe the admitted id from `connection.state().submitted_operation` while the gated run is active, then cancel that exact id. External `tests/operation_association.rs` remains responsible for public success/provider-failure/cardinality/PartialCommit evidence and does not invoke private cancellation. A pre-cancelled isolated Flow, parsed error string, synthetic terminal evidence, test-only production branch, public generic control, or shutdown-to-cancel coupling is insufficient. [VERIFIED: `mod.rs:343-357,1226-1315`; `operation_control.rs:92-226`; `intent_router.rs:37-104,166-188`; `public_projection.rs:407-428,557-604`; `manual_compaction_flow.rs:97-147,440-477,522-526`; `flow_service.rs:207-221`; `manual_compaction_service.rs:20-72`; locked D-06/D-07]
...
| Shutdown and operation cancellation orchestration | `coding_session/mod.rs`, `operation_control.rs`, `intent_router.rs`, `event_service.rs` | Admission close, drain, final lifecycle publication, receiver closure; pre-transfer crate-private `CompactCancellationHandle`, exact active id/generation validation, and Compact-only permit token without exposing generic client control or coupling shutdown to cancellation |
| Association ledger | `coding_session/public_operation.rs`, `operation.rs`, `event.rs`, `public_event.rs`, `manual_compaction_flow.rs`, `flow_service.rs`, `manual_compaction_service.rs`, `error.rs` | Closed 15-row classification, exact root terminal correlation, and typed live Compact cancellation semantics |
...
No user decision is required. The four former planning questions are resolved as follows:

1. **Lifecycle notification:** use `tokio::sync::Notify` paired with a monotonic lifecycle epoch stored under `SnapshotCoordinator.state`. Receivers/drain waiters snapshot the epoch, validate state, then select/recheck after notification so detach generation and runtime shutdown are distinguishable without missed wake-ups. Notification always occurs after releasing the standard mutex.
2. **Exact terminal evidence:** root-emission helpers return their exact sequenced `ProductEvent` and canonical finalization consumes that value directly. Do not store a second terminal-association index and do not scan bounded retention.
3. **Outcome acknowledgement identity:** expose an opaque `CodingAgentOutcomeAcknowledgementId` newtype minted by the existing session id-generation path at admitted OutcomeOnly finalization. Construction and inner authority remain private; submitted state carries the token, and acknowledgement validates connection generation plus exact token equality.
4. **Compact closure:** retain `TerminalAssociated`. Success emits same-id `Session.CompactionCompleted` and compatibility `Workflow.PromptCompleted`, but only the descriptor-permitted Compact-kind event counts. Failure and operation-scoped Flow cancellation both use the existing same-id `Workflow.PromptFailed` with `PromptTurnOutcome::Failed`; `CodingSessionError::Cancelled` distinguishes cancellation. Close the missing reachable path with a cloneable crate-private `CompactCancellationHandle` captured before the session owner moves. Admission stores an exact `{ operation_id, kind, generation, Compact token }` identity; `cancel(operation_id)` fails closed unless that id is the current Compact identity, the permit carries the token through `ManualCompactionOptions` to `ManualCompactionFlow::run_with_options`, and `FlowError::Cancelled` maps explicitly. Prove reachability in a colocated canonical-run lib test by observing the running submitted id through a retained connection; keep external association tests public-only. The descriptor normalizes only that failed compatibility branch to admitted kind Compact, preserving event inventory and adapter compatibility without a public generic control, shutdown cancellation, or aborted compatibility variant.
